---
layout: post
title:  "2020-12-25-查询CPU占用率过高的Java线程"
date:   2020-12-25 12:03:36 +0530
categories: Web
tags: [java, jvm]
description:
---
...
一般情况下，对于WEB服务器来说，CPU占用率过高的典型场景包括
- 计算密集，例如大量序列化，检索

- 死循环，这种情况一般CPU使用率会接近100%

- 资源申请泛滥，例如不加节制地新建线程，处于Runnable和Running状态的线程数过多

- 频繁GC，GC触发了大量的内存管理操作，会使CPU使用率变高

### 2. CPU使用率过高的排查思路

- 使用top命令查看CPU使用率最高的进程和线程  
    i top -H  
    ii 记下CPU使用率高的进程id和线程id  

- 查询线程栈  
    i jstack {进程id}  
    ii 手动转换线程id成十六进制（可以用printf %x {线程id}）  
    iii 在jstack输出中查找十六进制的线程id（grep 0x{线程id-十六进制} -A 20）  

- 查询线程堆栈，分析问题

### 3. bash脚本-show-busy-java-thread
